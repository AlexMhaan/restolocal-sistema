  ...
  methods: {
    ...
    obtenerIdElemento() {
     ...
     listarPlatillosCocina() {
      // Modificado para filtrar solo platillos de barra
      axios.get(BASE_URL + "api/test/?preparado=true&tipo=barra").then(responseItems => {
        console.log('Platillos de barra:', responseItems.data);
    ...
    actualizarEstado(platillo, nuevoEstado) {
      ...
      axios.put(BASE_URL + "api/test/" + platillo.id, datos)
        .then(response => {
          ...
          if (index !== -1) {
            // Siempre actualizar el estado localmente (no remover)
